...
const vehicleSchema = z.object({
  // modelo: z.string().nonempty("Modelo is required"),
  // marca: z.string().nonempty("Marca is required"),
  // ano: z.string().nonempty("Ano is required"),
  tempo: z.string().nonempty("A quilometragem deve ser preenchida"),
  preco: z.string().nonempty("O preço deve ser preenchido"),
  descricao: z.string().nonempty("A descrição deve ser preenchida"),
  // imageUrl: z.array(z.string()).nonempty("Image is required"),
  // user: z.any(),
});
...
const CadastroVeiculo = () => {
  ...
  const { register, handleSubmit, formState } = useForm<vehicleData>({
  ...
  const handleVehicle = async (data: vehicleData) => {
    ...
    if (yearsIdValue && nameBrandValue && nameModelValue) {
      try {
    ...
    <div>
      ...
      <div className=" flex flex-col justify-center items-center w-full 2xl:h-screen">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-xl font-semibold">Anunciar Veículo</p>
        </div>
        <form
          onSubmit={handleSubmit(handleVehicle)}
          className="flex flex-col justify-center items-center space-y-6 mt-6"
        >
          <div>
            ...
            <Popover open={openBrand} onOpenChange={setOpenBrand}>
              <PopoverTrigger asChild>
                ...
                >
                  {brandValue
                    ? brands.find((brands) => brands.codigo === brandValue)
                        ?.nome
                    : "Selecionar marca..."}
          ...
          <div>
            <div>
              ...
              ></Input>
              {formState.errors.tempo && <div className="text-red-400 absolute">{formState.errors.tempo.message}</div>}
            </div>
          </div>
          <div>
            <div>
              ...
              ></Input>
              {formState.errors.preco && <div className="text-red-400 absolute">{formState.errors.preco.message}</div>}
            </div>
          </div>
          <div className="pb-4">
            <div>
              ...
              <Input
                ...
                name="descricao"
              ></Input>
              {formState.errors.descricao && <div className="text-red-400 absolute">{formState.errors.descricao.message}</div>}
            </div>
          </div>
          <div>
            <CldUploadWidget
